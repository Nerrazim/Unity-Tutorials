...
public class LastPlayerSighting : MonoBehaviour {
	...
	{
		if (position != resetPosition) {
			backgroundAudio.volume = Mathf.Lerp (backgroundAudio.volume, 0f, musicFadeSpeed * Time.deltaTime);
			panicAudio.volume = Mathf.Lerp (panicAudio.volume, 0.8f, musicFadeSpeed * Time.deltaTime);
		} 
		else 
		{
			backgroundAudio.volume = Mathf.Lerp(backgroundAudio.volume, 0.8f, musicFadeSpeed * Time.deltaTime);
